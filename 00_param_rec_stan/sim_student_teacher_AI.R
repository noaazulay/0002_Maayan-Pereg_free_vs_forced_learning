...
sim.block_AI = function(par,cfg){ 
  ...
  trial = student.ch  =  instructions.rate = student.ob = student.obey = teacher.ch = teacher.advice = reward = beta.teacher = teacher.acc  = teacher.reveal = rep(NA, Ntrl)
  ...
  for (t in 1:Ntrl){
    ...
    #Teacher learning
    #PE.teacher = ((reward[t]*student.ob[t])-Qinst[(teacher.reveal[t]==0)*1+1])
    PE.teacher = (reward[t]-Qinst[(teacher.reveal[t]==0)*1+1])
    
    #associability[student.ch[t]] = w*abs(PE.teacher) + (1-w)*associability[student.ch[t]] #associability weights how much the student already learned, with the absolute PE (this allows the teacher to halt insttructions when the student already learned enough)
    associability[(teacher.reveal[t]==0)*1+1] = w*abs(PE.teacher) + (1-w)*associability[(teacher.reveal[t]==0)*1+1] #associability weights how much the student already learned, with the absolute PE (this allows the teacher to halt insttructions when the student already learned enough)
    
    #Q values the teacher holds, this is Q for giving/not giving instructions:
    Qinst[(teacher.reveal[t]==0)*1+1]=Qinst[(teacher.reveal[t]==0)*1+1]+alpha.teacher*associability[student.ch[t]]*((reward[t]*student.ob[t])-Qinst[(teacher.reveal[t]==0)*1+1])
    #Qinst[(teacher.reveal[t]==0)*1+1]=Qinst[(teacher.reveal[t]==0)*1+1]+alpha.teacher*associability[student.ch[t]]*(reward[t]-Qinst[(teacher.reveal[t]==0)*1+1])
    
    #based on Qinst, what is the probability to give instructions in the next trial
    pInst <- exp(beta.teacher.exp*Qinst)/sum(exp(beta.teacher.exp*Qinst))
    
    trial[t]=t
    
  }
  
  return (data.frame(trial,teacher.ch,teacher.reveal,student.ch,student.ob,reward,Ntrl,cards.reveal.exp,teacher.rate,instructions.rate,w))
}

sim.block_AI_2 = function(par,cfg){ 
  Nalt          = cfg$Nalt
  Ntrl          = cfg$Ntrl
  rndwlk_teacher= cfg$rndwlk_teacher
  teacher.rate  = cfg$teacher.rate #initial instructions rate
  ...
  for (t in 1:Ntrl){
    ...
    #Teacher learning -without Pearce-Hall
    #PE.teacher = ((reward[t]*student.ob[t])-Qinst[(teacher.reveal[t]==0)*1+1])
    #associability[student.ch[t]] = w*abs(PE.teacher) + (1-w)*associability[student.ch[t]] #associability weights how much the student already learned, with the absolute PE (this allows the teacher to halt insttructions when the student already learned enough)
    #Q values the teacher holds, this is Q for giving/not giving instructions:
    
    Qinst[(teacher.reveal[t]==0)*1+1]=Qinst[(teacher.reveal[t]==0)*1+1]+alpha.teacher*((reward[t]*student.ob[t])-Qinst[(teacher.reveal[t]==0)*1+1])
    #Qinst[(teacher.reveal[t]==0)*1+1]=Qinst[(teacher.reveal[t]==0)*1+1]+alpha.teacher*(reward[t]-Qinst[(teacher.reveal[t]==0)*1+1])
  ...
  return (data.frame(trial,teacher.ch,teacher.reveal,student.ch,student.ob,reward,Ntrl,cards.reveal.exp,teacher.rate,instructions.rate))
}
