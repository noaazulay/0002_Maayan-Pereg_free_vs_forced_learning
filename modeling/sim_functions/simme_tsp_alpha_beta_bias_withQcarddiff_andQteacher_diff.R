...
sim.block = function(par,cfg){
  ...
  bias_intercept    = par['bias_intercept']
  bias_slope1       = par['bias_slope1']
  bias_slope2       = par['bias_slope2']
  beta              = exp(par['beta'])
...
for (block in 1:Nblocks) {
  ...
  Qteacher      = c(0,0)       #follow or oppose
    ...
    Qnet= c(0,0)
    Qnet= Qcard[raffle]
    bias= bias_intercept+bias_slope1*abs(Qnet[1]-Qnet[2])+bias_slope2*(Qteacher[2]-Qteacher[1])
    ...
    PEteacher          = reward-Qteacher[follow+1]
    ...
    Qteacher[follow+1] = Qteacher[follow+1] +alpha*PEteacher
