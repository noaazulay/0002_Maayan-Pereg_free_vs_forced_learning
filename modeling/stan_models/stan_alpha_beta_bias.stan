data {
  ...
  int<lower = 0, upper = 1> reveal[Nsubj,Ntrials]; 
  int<lower = 0, upper = 1> follow[Nsubj,Ntrials]; 
  int<lower = 0> offer1[Nsubj,Ntrials]; 
  int<lower = 0> offer2[Nsubj,Ntrials]; 
  int<lower = 0> raffle_student_ch[Nsubj,Ntrials]; 
  int<lower = 0> raffle_teacher_ch[Nsubj,Ntrials]; 
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  for (subj in 1:Nsubj){
        ...
        bias[subj]               = (auxiliary_parameters[subj][2]); 
        beta[subj]              = exp(auxiliary_parameters[subj][3]);
        ...
        for (trial in 1:Ntrials){
            ...
            Qnet[raffle_teacher_ch[subj,trial]]=Qnet[raffle_teacher_ch[subj,trial]]+bias[subj];
            
            //print(offer1[subj,trial]);
            log_lik[subj,trial]=log_softmax(Qnet*beta[subj])[raffle_student_ch[subj,trial]];
